...
export default function RecurringTransactions({ transactions }: Props) {
  const recurring = useMemo(() => {
    ...
    transactions.forEach(t => {
      if (t.amount >= 0) return; // Kun udgifter
    ...
    // Find dem der optræder 2+ gange (recurring)
    ...
    <div className="bg-white rounded-3xl shadow-xl p-8 border border-gray-100">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">Automatisk genkendt</p>
        </div>
      </div>
      ...
      <div className="mt-6 text-center text-sm text-gray-500">
        Disse udgifter dukker op flere gange – overvej at sætte dem som faste i dit budget
      </div>
    </div>
  );
}