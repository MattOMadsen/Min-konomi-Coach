// 1 fil - 1 funktion
// parsers/banks/nordeaParser.ts – Dedikeret parser kun til Nordea CSV (header + specifikke kolonner)

import Papa from 'papaparse';
import type { Transaction } from '../csvParser';

export const parseNordeaCSV = (csvText: string): Transaction[] => {
...
// Intern row-parser kun til Nordea-formatet
const parseNordeaRow = (row: Record<string, any>): Transaction | null => {
  ...
  const amountRaw = row.Beløb || row.Amount || row.Sum || row.Belob || '';
